...
function predictMatch($match, $apiKey, $baseUrl, &$teamStats, $competition) {
    try {
        ...
        // Adjusted Home/Away Adjustments
        $homeStrengthAdjustment = 1.05 + ($homeHomeWins / max($homeGames / 2, 1)) * 0.03;
        $awayStrengthAdjustment = 0.95 - ($awayAwayWins / max($awayGames / 2, 1)) * 0.01;

        $competitionFactor = match ($competition) {
            'UEFA Champions League' => 1.15,
            'English Championship' => 1.0,
            default => 1.05
        };

        // Momentum and Form (Increased draw weight)
        ...
        $momentumWeights = [0.15, 0.25, 0.45]; // Increased weight for recent games
        $homeMomentum = $awayMomentum = 0;
        for ($i = 0; $i < 3 && $i < count($homeFormArray); $i++) {
            $result = array_reverse($homeFormArray)[$i];
            $homeMomentum += ($result === 'W' ? 3 : ($result === 'D' ? 2 : 0)) * $momentumWeights[$i]; // Draw weight up from 1.5 to 2
        }
        for ($i = 0; $i < 3 && $i < count($awayFormArray); $i++) {
            $result = array_reverse($awayFormArray)[$i];
            $awayMomentum += ($result === 'W' ? 3 : ($result === 'D' ? 2 : 0)) * $momentumWeights[$i];
        }
        $momentumFactor = 1 + ($homeMomentum - $awayMomentum) * 0.04;

        $homeOpponentStrength = min(1.15, max(0.85, 1 - (($homePosition - ($totalTeams / 2)) / ($totalTeams / 2)) * 0.08));
        $awayOpponentStrength = min(1.15, max(0.85, 1 - (($awayPosition - ($totalTeams / 2)) / ($totalTeams / 2)) * 0.08));

        $formWeightBase = [0.05, 0.10, 0.15, 0.25, 0.35, 0.45]; // Increased weights for recent form
        $homeFormScore = $awayFormScore = 0;
        foreach (array_reverse($homeFormArray) as $i => $result) {
            $homeFormScore += ($result === 'W' ? 3 : ($result === 'D' ? 2 : 0)) * $formWeightBase[$i] * $homeOpponentStrength;
        }
        foreach (array_reverse($awayFormArray) as $i => $result) {
            $awayFormScore += ($result === 'W' ? 3 : ($result === 'D' ? 2 : 0)) * $formWeightBase[$i] * $awayOpponentStrength;
        }

        // Dynamic Weights (Increased draw weight)
        $maxWeight = 100;
        $winWeight = min(10, 8 + ($homeGames + $awayGames) * 0.06); // Reduced slightly
        $drawWeight = min(25, 18 + ($homeGames + $awayGames) * 0.15); // Increased from 15 to 18 base
        $goalWeight = min(25, 20 + (abs($homeGD) + abs($awayGD)) * 0.02 / max($homeGames, $awayGames));
        $standingsWeight = min(18, 12 + ($homeGames + $awayGames) * 0.10);
        $formWeight = min(35, 22 + (strlen(trim($homeStats['form'] ?? '', '-')) + strlen(trim($awayStats['form'] ?? '', '-'))) * 0.6); // Increased base
        ...
        $diff = $homeStrength - $awayStrength + ($totalTeams - $homePosition) * 0.02 - ($totalTeams - $awayPosition) * 0.02;
        $totalStrength = $homeStrength + $awayStrength + 1;

        // Enhanced Draw Detection
        $drawLikelihood = ($homeDrawRate + $awayDrawRate) * $drawWeight * 1.5; // Increased from 1.2 to 1.5
        $diffThreshold = 0.5; // Reduced from 0.8 to widen draw range
        $drawFormIndicator = (substr_count($homeStats['form'] ?? '', 'D') + substr_count($awayStats['form'] ?? '', 'D')) / 8.0;

        // Form Variance for Confidence Adjustment
        $homeFormVariance = array_reduce($homeFormArray, fn($carry, $r) => $carry + ($r === 'W' ? 1 : ($r === 'D' ? 0 : ($r === 'L' ? -1 : 0))), 0) / max(1, count(array_filter($homeFormArray, fn($r) => $r !== '-')));
        $awayFormVariance = array_reduce($awayFormArray, fn($carry, $r) => $carry + ($r === 'W' ? 1 : ($r === 'D' ? 0 : ($r === 'L' ? -1 : 0))), 0) / max(1, count(array_filter($awayFormArray, fn($r) => $r !== '-')));
        $formStability = 1 - (abs($homeFormVariance) + abs($awayFormVariance)) / 3;
        ...
        $strengthGap = abs($homeStrength - $awayStrength);
        $confidenceBase = 45 + ($strengthGap / $totalStrength * 40) * $formConsistency * $formStability + $goalDiffFactor * 6;
        $confidence = min(75, max(40, $confidenceBase)); // Increased max from 70 to 75

        // Enhanced Goal Prediction
        ...
        $homeAttackStrength = min(2.0, ($homeGoalTrend * 0.7 + $homeGoalAvg * 0.3) * $homeOpponentStrength);
        $awayAttackStrength = min(2.0, ($awayGoalTrend * 0.7 + $awayGoalAvg * 0.3) * $awayOpponentStrength);
        $homeDefStrength = min(1.8, ($homeConcededAvg * 0.7 + 0.3) * $awayOpponentStrength);
        $awayDefStrength = min(1.8, ($awayConcededAvg * 0.7 + 0.3) * $homeOpponentStrength);

        $expectedHomeGoals = max(0, min(4.0, ($homeAttackStrength * 0.8 + $homeGoalTrend * 0.2) * (1 + $diff / 40) / ($awayDefStrength + 0.3) * $competitionFactor));
        $expectedAwayGoals = max(0, min(4.0, ($awayAttackStrength * 0.8 + $awayGoalTrend * 0.2) * (1 - $diff / 40) / ($homeDefStrength + 0.3) * $competitionFactor));

        // Adjust goals for draws
        if ($drawLikelihood > 0.25 || abs($diff) <= $diffThreshold) { // Lowered from 0.35 to 0.25
            $goalDiff = abs($expectedHomeGoals - $expectedAwayGoals);
            if ($goalDiff > 0.5) { // Reduced threshold for equalization
                $avgGoals = ($expectedHomeGoals + $expectedAwayGoals) / 2;
                $expectedHomeGoals = $avgGoals;
                $expectedAwayGoals = $avgGoals;
            }
        }
        ...
        // Prediction Logic
        if ($diff > 0.8) { // Tightened from 1.0 for clearer wins
            $prediction = "$homeTeam to win";
            $advantage = "Home Advantage";
        } elseif ($diff < -0.8) {
            $prediction = "$awayTeam to win";
            $advantage = "Away Advantage";
        } elseif (abs($diff) <= $diffThreshold && ($drawLikelihood > 0.25 || $drawFormIndicator > 0.35)) { // Adjusted thresholds
            $prediction = "Draw";
            $advantage = "Tight Match";
            $confidence = min($confidence, 65); // Increased cap from 60 to 65
            if ($predictedHomeGoals != $predictedAwayGoals) {
                $predictedHomeGoals = $predictedAwayGoals = min($predictedHomeGoals, $predictedAwayGoals) + rand(0, 1);
                $predictedScore = "$predictedHomeGoals-$predictedAwayGoals";
            }
        } else {
            $prediction = ($diff > 0) ? "$homeTeam to win" : "$awayTeam to win";
            $advantage = ($diff > 0) ? "Slight Home Edge" : "Slight Away Edge";
...
function enforceClientRateLimit(callback, args) {
    const now = Date.now();
    
    // Clean up old timestamps
    requestTimestamps = requestTimestamps.filter(ts => now - ts < MINUTE_IN_SECONDS * 1000);
...
function processRequestQueue() {
    ...
    const toProcess = requestQueue
        .filter(req => req.scheduledTime <= now)
...
function fetchTeamData(teamId, index, isHome, attempt = 0, maxAttempts = 10) {
    enforceClientRateLimit(fetchTeamDataImpl, [teamId, index, isHome, attempt, maxAttempts]);
}

function fetchTeamDataImpl(teamId, index, isHome, attempt, maxAttempts) {
    ...
    console.log(`Fetching team data for ${teamId} (attempt ${attempt})`);
    ...
    .then(data => {
        if (data.queued) {
            ...
            setTimeout(() => fetchTeamData(teamId, index, isHome, attempt), data.delay * 1000);
    ...
    enforceClientRateLimit(fetchPredictionImpl, [index, homeId, awayId, attempt, maxAttempts]);
}

function fetchPredictionImpl(index, homeId, awayId, attempt, maxAttempts) {
    ...
    console.log(`Fetching prediction for match ${index} (attempt ${attempt})`);
    ...
    .then(data => {
        if (data.queued) {
            ...
            setTimeout(() => fetchPrediction(index, homeId, awayId, attempt), data.delay * 1000);
        } else if (data.success) {
            ...
            predictionElement.innerHTML = '<p>Error loading prediction</p>';
            tablePrediction.innerHTML = 'Error';
            tableConfidence.innerHTML = '';
            tablePredictedScore.innerHTML = '';
            progressBar.parentElement.remove();
            predictionElement.querySelector('.retry-notice')?.remove();
    ...
    .catch(error => {
        ...
        if (attempt < maxAttempts) {
            ...
            predictionElement.innerHTML = '<p>Failed to load prediction</p>';
            tablePrediction.innerHTML = 'Failed';
            tableConfidence.innerHTML = '';
            tablePredictedScore.innerHTML = '';
            progressBar.parentElement.remove();
            predictionElement.querySelector('.retry-notice')?.remove();
...
function startMatchPolling() {
    ...
    pollingInterval = setInterval(() => {
        processRequestQueue(); // Process any queued requests first
        ...
        matches.forEach(element => {
            ...
            fetchPrediction(index, homeId, awayId); // Use rate-limited version
            requestsMade++;
        });
