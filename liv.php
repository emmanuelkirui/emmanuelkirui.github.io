...
$competitions_url = $base_url . "competitions"; // List all competitions
...
// Function to log errors (optional enhancement)
...
function initializeData($api_key, &$state) {
    if (empty($_SESSION['competitions'])) {
...
if (!isset($_GET['ajax'])) {
    ...
    // Inline CSS for a more professional look (original styling preserved)
    echo "<style>
        /* Navbar container */
        .navbar {
            width: 100%;
...
// Get selected competition and filters
$selected_competition = isset($_GET['competition']) ? $_GET['competition'] : 'PL';
$search_team = isset($_GET['team']) ? $_GET['team'] : '';
...
// Hide the status light after page load
...
if ($selected_competition && $fixtures_data) {
    ...
    if ($search_team) {
        $filtered_matches = searchMatchesByTeam($filtered_matches, $search_team);
    }
