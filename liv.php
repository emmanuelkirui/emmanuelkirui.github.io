...
if (!isset($_GET['ajax'])) {
    ...
    // Inline CSS for professional styling with auto-adjustment
    echo "<style>
        /* Navbar container */
        .navbar {
            width: 100%;
...
// Helper function to calculate recent form weight
...
// Function to get last 6 matches
...
function searchMatchesByTeam($matches, $team) {
    $filtered_matches = [];
    foreach ($matches as $match) {
        if (stripos($match['homeTeam']['name'], $team) !== false || stripos($match['awayTeam']['name'], $team) !== false) {
            $filtered_matches[] = $match;
        }
    }
    return $filtered_matches;
...
// Get selected competition and fetch data
...
if ($selected_competition) {
    ...
    $fixtures_data = $team_metrics = $standings = null;
}
...
echo '<div id="status_light" class="status">
...
echo '<script>
    ...
    window.onload = function () {
        document.getElementById("status_light").style.display = "none";
    };
</script>';
...
echo "<script>
    function toggleCustomRange(value) {
        document.getElementById('custom_range').style.display = value === 'custom' ? 'block' : 'none';
    }
</script>";
            ...
            canvas.toBlob(blob => {
                ...
                if (navigator.share && navigator.canShare({ files: [file] })) {
                    navigator.share({ files: [file], title: "Captured Table", text: "Here is a table snapshot" })
                        ...
                        .catch(error => showFeedbackMessage("Failed to share table.", false));
                } else {
